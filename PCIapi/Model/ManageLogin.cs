...
public ManageLogin(IConfiguration configuration) : base(configuration)
 ...
 ;           }
        ...
        public UserModel getUsers( string UserName,  string Password)
        {
            using (IDbConnection dbConnection = Connection)
            {
                string sQuery = @"SELECT  EmailID, UserName,IsReset,IsBlocked,EmployeeID from MstLogintbl Where (UserName=@_strUsername OR EmployeeID = @_strUsername) AND Password=@_strPassword";
                dbConnection.Open();
